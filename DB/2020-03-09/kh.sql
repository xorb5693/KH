...
SELECT E1.EMP_ID, E1.EMP_NAME, E1.MANAGER_ID, NVL(E2.EMP_NAME, '없음') AS "관리자 이름"
FROM EMPLOYEE E1
LEFT JOIN EMPLOYEE E2 ON (E2.EMP_ID = E1.MANAGER_ID)
ORDER BY 1;

--스칼라 서브쿼리
--위의 문제를 스칼라 서브쿼리로 재실행
...
--3. 매니저가 있는 사원중에 월급이 전체사원 평균을 넘는 사원의 
--사번, 이름, 매니저 이름, 월급(만원 단위부터)을 구하시오
SELECT E1.EMP_ID AS "사번", E1.EMP_NAME AS "이름", (
    SELECT E2.EMP_NAME
    FROM EMPLOYEE E2
    WHERE E1.MANAGER_ID = E2.EMP_ID) AS "매니저 이름", 
TRUNC(E1.SALARY, -4) AS "급여"
FROM EMPLOYEE E1
...
FROM EMPLOYEE E1
WHERE (JOB_CODE, SAL_LEVEL) = (
    SELECT E2.JOB_CODE, E2.SAL_LEVEL
    FROM (
        SELECT * FROM EMPLOYEE
        ORDER BY SAL_LEVEL
    ) E2
